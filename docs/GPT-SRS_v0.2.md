...
- 사장님이 쉽게 스케줄을 만들고, 조정하고, 승인할 수 있도록 한다.
...
#### 1.2.1 In-Scope (MVP v1.0)

MVP v1.0에서 시스템이 제공해야 하는 범위는 다음과 같다.

- **3단계 온보딩 마법사 (F-01)**  
  - 사장님 회원가입 및 인증  
  - 매장 정보 설정(업종, 영업시간, 대리인 정보 등)  
  - 첫 주간 스케줄 생성까지의 흐름 제공  
  - 제품 KPI로서, 신규 매장의 70%가 10분 이내에 온보딩과 첫 스케줄 생성을 완료하는 것을 목표로 한다(참고 지표로 사용하며, 기능 결함 판정의 유일 기준은 아님).
...
- **사장님 1클릭 스케줄 승인 및 공지 (F-03, Story 2)**  
  ...
  - 승인된 스케줄 또는 변경된 스케줄을 직원에게 **하나의 채널(초기에는 카카오톡 기반 메시지 우선)** 으로 통지한다. SMS는 비용 및 운영 정책에 따라 선택적으로 확장한다.

- **노동법 규칙 엔진 및 사전 차단 (F-04, Story 1)**  
  - 주 52시간, 휴게시간, 연속 휴식, 주휴수당, 야간·휴일 근로 등 핵심 규칙을 스케줄에 자동 적용  
  - 위반이 존재할 경우 승인 버튼을 비활성화하고, **어떤 규칙을 왜 위반했는지**, 어떻게 수정해야 하는지 가이드를 제공  
  - MVP v1.0에서는 사전에 정의된 규칙 세트 및 단순 파라미터 기반 엔진을 사용하고, 고급 규칙 관리(CSV Import/Export, 복잡한 공식 편집)는 단계적으로 확장한다.
...
- **변경 이력 기록 (Audit Log v1, F-06)**  
  ...
  - MVP v1.0에서는 수행자, 시각, 주요 변경 내용(전/후)을 우선 저장하고, SHA-256 서명 등 강화된 변조 검출은 단계적 도입 대상으로 간주한다.
...
- **API 및 인터페이스 (PRD §6)**  
  ...
  - 스케줄 조회/승인 API  
  - 급여 리포트 조회 API  
  - (확장용) 노무사 파트너에게 보내는 법규 위반 Webhook  
  - (확장용) 노무사 전용 법규 규칙 CSV 가져오기/내보내기 인터페이스  
  - Webhook 및 CSV 기반 규칙 관리는 MVP v1.0에서는 구현 우선순위가 낮으며, 초기 출시 이후(v1.1+) 확장 대상이다.

> **MVP Implementation Note**  
> 본 SRS의 기능 요구 중 `Priority = Must`인 항목은 **MVP v1.0 필수 구현 범위**이다.  
> `Priority = Should / Could`인 항목은 단계적 롤아웃(v1.1+) 대상으로 간주하며, 개발 일정·리소스에 따라 우선순위를 재조정할 수 있다.

#### 1.2.2 Out-of-Scope (MVP v1.0 제외 범위)

다음 기능은 MVP v1.0에서는 제공하지 않는다(향후 버전에서 고려).
...
- **인프라 및 데이터 위치 제약**  
  - 데이터 저장·처리는 **국내 리전(AWS 서울 리전 등)** 에서만 수행되고, 해외 리전은 사용하지 않는다.  
  - 직원 PII는 GDPR 및 국내 법령의 최소 수집 원칙을 준수하며, 직원 1인당 수집 필드는 8개 이하로 제한한다.

- **보안 제약**  
  - 저장 시 모든 PII 및 민감 데이터를 **AES-256**으로 암호화하는 것을 목표로 한다.  
  - 클라이언트 및 외부 시스템과의 통신은 **TLS 1.3 이상**만 허용하는 것을 목표로 하되, 초기 MVP 내부 검증 단계에서는 클라우드 제공자의 기본 HTTPS 구성을 사용할 수 있다.  
  - 규칙 관리·내부 관리자 기능에는 **MFA(다중 인증)** 를 사용한다(베타/정식 론칭 전까지 적용).

- **운영 의존성**  
  - 카카오톡/SMS 게이트웨이는 99.9% 이상의 가동률을 유지하고, 장애 발생 시 4시간 이내 복구된다고 가정한다.  
  - 모니터링·알림은 Datadog, PagerDuty **또는 동등 수준의 클라우드 네이티브 서비스(AWS CloudWatch, SNS/SQS 등)** 를 사용하여 구현 가능하다고 가정한다.
...
| **Audit Log**          | 주요 엔터티 변경 기록(누가, 언제, 무엇을, 어떻게 바꿨는지와 서명)을 담은 로그                   |
...
### 1.5 Technology Stack Assumptions (C-TEC, 기술 스택 가정)

본 시스템의 초기 MVP v1.0 구현은 다음 기술 스택 가정을 따른다.

- **C-TEC-001: 프론트엔드 (React + Vite)**  
  - 모든 프론트엔드 서비스(사장님 웹 대시보드, 직원용 모바일 폼, 관리자·노무사 콘솔)는 Vite 기반 **React.js** 프레임워크를 사용한다.  
  - 초기에는 단일 프론트엔드 프로젝트로 시작하며, 역할별 화면은 라우팅/권한 제어로 구분한다.

- **C-TEC-002: 백엔드 (Java 17 + Spring Boot 3.x)**  
  - 모든 핵심 백엔드 API는 JVM 기반 **Java 17 + Spring Boot 3.x**로 구현한다.  
  - MVP v1.0에서는 규칙 엔진, 스케줄 관리, 급여 계산, 알림 트리거, Audit Log를 포함하는 **단일 모놀리식 서비스**로 시작한다.

- **C-TEC-003: 데이터베이스 (MySQL 8.x InnoDB)**  
  - 데이터베이스는 **MySQL 8.x** 버전의 InnoDB 엔진을 사용하며, 유니코드 확장 문자 인코딩(예: `utf8mb4`)을 기본으로 사용한다.  
  - `Store`, `Employee`, `Schedule`, `ComplianceRule`, `AuditLog` 등 주요 엔터티를 RDB 스키마로 관리한다.

- **C-TEC-004: 문서 자동생성 엔진 (Python + FastAPI + LangChain)**  
  - 증빙 문서 및 일부 리포트 자동생성은 별도의 경량 서비스로 구현하며, **Python 기반(FastAPI + LangChain)** 을 사용한다.  
  - Spring Boot 백엔드에서 HTTP REST 호출로 이 서비스에 요청을 전달하고, PDF/텍스트 결과를 받아 오브젝트 스토리지에 저장한다.

- **C-TEC-005: LLM 호출 (OpenAI API / LLM Gateway, 예: Google Gemini)**  
  - LLM 호출은 OpenAI API 또는 사내/외부 LLM Gateway를 사용하며, Google Gemini 등 호환 가능한 모델을 사용 가능하다.  
  - LangChain 레이어에서 LLM 공급자별 차이를 추상화한다.

- **C-TEC-006: 서비스 간 통신 방식 (REST / gRPC)**  
  - MSA 서비스 간 통신은 REST(OpenAPI 3.x) API 또는 gRPC로 제한한다.  
  - **MVP v1.0에서는 Spring Boot 모놀리식 서비스와 Python 문서 생성 서비스 간 REST(JSON) 통신만을 사용**하며, gRPC는 향후 서비스 분리 시 선택적으로 도입한다.
...
| EXT-04 | 모니터링 서비스                 | 성능·에러·큐 상태 등을 수집하는 모니터링 시스템 (예: Datadog, AWS CloudWatch 등)      | HTTPS API / 에이전트         |
| EXT-05 | 장애관리 시스템                 | SLA·성능 위반 발생 시 알림을 전달하는 장애 관리 시스템 (예: PagerDuty, SNS 등)       | HTTPS API / Webhook          |
...
| API-05 | POST   | `/events/violation` (Webhook)             | 서명 헤더        | 이벤트 기반            | 중대 법규 위반 발생 시 노무사에게 알림 (MVP v1.0에서는 Could, v1.1+에서 활성화 가능)     |
| API-06 | POST   | `/admin/rules/import`                     | 노무사 인증      | 제한                   | 노동 규칙 CSV 업로드·적용 (MVP v1.0에서는 Should, 단계적 롤아웃)                          |
...
sequenceDiagram
    ...
    Owner->>Svc: (옵션) 시간 조정 UI를 통해 교대 시간 조정
...
일부 보안·운영 관련 요구사항은 **베타/정식 론칭 전까지 달성해야 하는 목표 상태(Target State)** 로 간주한다.
...
| REQ-NF-007  | 보안 (목표 상태)| 저장 데이터 암호화                                                                  | 모든 PII 및 민감한 급여 데이터는 AES-256으로 암호화된 상태로 저장되어야 한다.                                     | 보안 감사, 구성 점검                     |
| REQ-NF-008  | 보안 (목표 상태)| 전송 구간 암호화                                                                    | 모든 외부 HTTP 엔드포인트는 TLS 1.3 이상만 허용하고, HTTP 요청은 차단 또는 HTTPS로 리다이렉트해야 한다.           | 모의 침투 테스트, 설정 검증             |
...
| REQ-NF-011  | 보안 (목표 상태)| 관리자·규칙 관리 접근 제어                                                          | 규칙 관리·내부 관리자 기능은 인증된 계정과 MFA를 통해서만 접근 가능해야 한다.                                     | 접근 제어 테스트, 보안 감사              |
| REQ-NF-012  | 비용            | 매장당 월 운영 비용                                                                 | 활성 매장 1곳당 평균 월 운영 비용이 3,000원 이하를 유지해야 한다.                                                 | 비용 모니터링, 재무 리포트               |
| REQ-NF-013  | 모니터링        | 실시간 모니터링 범위                                                                | 모니터링 도구(예: Datadog, AWS CloudWatch 등) 대시보드에 API 응답 시간, 규칙 엔진 실패, 알림 큐 깊이가 실시간에 가깝게 표시되어야 한다. | 대시보드 점검, 모니터링 설정 테스트      |
| REQ-NF-014  | 모니터링        | 알림 임계값                                                                         | API p95 응답 시간이 1.2초를 초과하는 상태가 5분 이상 지속될 경우, 심야 포함 장애 알림(PagerDuty, SNS 등)이 발생해야 한다. | 알림 시뮬레이션, 장애 대응 훈련          |
